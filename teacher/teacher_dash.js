...
(() => {
  ...
  let seed =
    JSON.parse(localStorage.getItem("teacherDashboardDB_v3")) ||
    JSON.parse(localStorage.getItem("teacherDashboardDB_v4"));
    ...
    seed || {
      ...
      courseMaterials: [
        ...
        },
        {
          ...
          courseId: "c1",
          type: "notes",
          title: "Lecture 1: Intro to Big O",
          url: "#",
        },
      ],
      assignments: [
      ...
      announcements: [
        {
          ...
          content: "The mid-term exam for CS201 will be held on October 10th.",
          date: "2025-09-01",
        },
        {
          id: "an2",
          ...
          content: `The college will be closed on ${new Date().getFullYear()}-10-02 for Gandhi Jayanti.`,
          date: "2025-09-03",
        },
      ],
      messages: [
        {
          ...
          history: [
            {
              from: "s101",
              text: "Sir, I have a doubt about the last lecture.",
            },
            { from: "t01", text: "Sure, Arjun. Please specify the topic." },
          ],
        },
      ],
    };
  ...
  const state = {
    ...
    get myCourses() {
      return db.courses.filter((c) => c.teacherId === LOGGED_IN_TEACHER_ID);
    },
  ...
  const cleanupViewState = () => {
    ...
    if (state.ui.liveSession.timerInterval)
      clearInterval(state.ui.liveSession.timerInterval);
    if (state.ui.liveSession.simulationInterval)
    ...
    $$("#mainNav .nav-item").forEach((item) => {
      ...
      item.classList.toggle(
        ...
        item.dataset.nav === view ||
          (isCourseDetail && item.dataset.nav === "my_classes") ||
          (isLiveSession && item.dataset.nav === "my_classes")
      );
    });
  };
  ...
  const openModal = (title, fields, onSubmit) => {
    ...
    fields.forEach((f) => {
      ...
      wrap.innerHTML = `
        ...
        ${
          f.type === "textarea"
            ? `<textarea id="${id}" name="${f.name}" placeholder="${
                f.placeholder || ""
              }">${f.value ?? ""}</textarea>`
            : f.type === "select"
  ...
  const mainViews = {
    ...
    student_profile: (studentId, courseId) =>
      renderStudentProfile(studentId, courseId),
  ...
  const renderDashboard = () => {
    ...
    const pendingSubmissions = db.submissions.filter((s) => {
      ...
      return (
        myCourses.some((c) => c.id === assignment.courseId) &&
        s.grade === null &&
        s.submittedAt !== null
      );
    });
    ...
    byId("contentArea").innerHTML = `
      <div class="grid-3">
        <div class="stat-card"><div class="icon"><span class="material-icons-outlined">class</span></div><div><div class="value">${
          myCourses.length
        }</div><div class="label">Assigned Classes</div></div></div>
        <div class="stat-card"><div class="icon"><span class="material-icons-outlined">groups</span></div><div><div class="value">${
          new Set(myCourses.flatMap((c) => c.studentIds)).size
        }</div><div class="label">Total Students</div></div></div>
        <div class="stat-card"><div class="icon"><span class="material-icons-outlined">pending_actions</span></div><div><div class="value">${
          pendingSubmissions.length
        }</div><div class="label">Pending Evaluations</div></div></div>
      </div>
      <div class="grid-2" style="margin-top: 1.5rem;">
        ...
        <div class="card">
          ...
          ${db.announcements
            ...
            .map(
              (a) =>
                `<div class="announcement-card"><div class="meta">${fmtDate(
                  a.date
                )} | ${
                  a.courseId ? getCourse(a.courseId).code : "College"
  ...
  const renderMyClasses = () => {
    ...
    `;
    // Actions
    byId("contentArea").addEventListener(
      "click",
      (e) => {
  ...
  const renderLiveSession = (courseId) => {
    ...
    state.ui.liveSession.qrCodeData = `SMARTAPP_ATTENDANCE::${course.id}::${uid(
      "sess"
    )}`;
    ...
    const qrApiUrl = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(
      state.ui.liveSession.qrCodeData
    )}`;
  ...
  const renderCourseDetail = (courseId) => {
    ...
    byId("contentArea").innerHTML = `
      <div class="tabs">
        ${["materials", "roster", "assignments", "grades"]
          .map(
            (t) => `
          <button class="${t === tab ? "active" : ""}" data-tab="${t}">${
  ...
  const renderCourseTab = (tab, courseId) => {
    ...
    if (tab === "materials") {
          ...
          <tbody>
            ${mats
              ...
              <tr>
                ...
                <td>${
                  m.url ? `<a href="${m.url}" target="_blank">Open</a>` : ""
                }</td>
      ...
      byId("btnAddMat").onclick = () =>
        openModal(
          ...
          (data) => {
            ...
            renderCourseTab("materials", courseId);
          }
        );
      wrap.addEventListener(
        ...
        (e) => {
          ...
          if (edit) {
            const m = db.courseMaterials.find(
          ...
          if (del) {
            db.courseMaterials = db.courseMaterials.filter(
    ...
    if (tab === "roster") {
      ...
      `;
      byId("btnMsgClass").onclick = () => {
        state.ui.messages.studentId = null;
        navigate("messages");
      };
      wrap.addEventListener(
        "click",
        (e) => {
          ...
          if (m) {
            state.ui.messages.studentId = m.dataset.message;
            navigate("messages");
          }
        },
        { once: true }
          ...
          <tbody>
            ${as
              .map((a) => {
                ...
                return `<tr><td>${a.title}</td><td>${fmtDate(
                  a.dueDate
                )}</td><td>${submitted}/${course.studentIds.length}</td>
                <td>
      ...
      byId("btnAddAssn").onclick = () =>
        openModal(
          ...
          (data) => {
            ...
            renderCourseTab("assignments", courseId);
          }
        );
      wrap.addEventListener(
        "click",
    ...
    if (tab === "grades") {
      ...
      const courseAssnIds = db.assignments
      ...
      const rows = students.map((s) => {
        const subs = db.submissions.filter(
          (sub) =>
            sub.studentId === s.id &&
            courseAssnIds.includes(sub.assignmentId) &&
            sub.grade != null
        );
        const avg = subs.length
          ? Math.round(subs.reduce((a, b) => a + b.grade, 0) / subs.length)
          : "-";
        ...
        <table class="table" id="gradesTable">
          ...
          <tbody>
            ${rows
              .map(
                (r) =>
                  `<tr><td>${r.name}</td><td>${r.email}</td><td>${r.avg}</td></tr>`
              )
              .join("")}
          </tbody>
        </table>
      ...
      byId("btnExportCSV").onclick = () => {
        const csv = ["Name,Email,Average"]
          .concat(rows.map((r) => `${r.name},${r.email},${r.avg}`))
          .join("\n");
  ...
  const renderStudentProfile = (studentId, courseId) => {
    ...
    const assnIds = db.assignments
      .filter((a) => a.courseId === courseId)
      .map((a) => a.id);
    const subs = db.submissions.filter(
      (x) => x.studentId === studentId && assnIds.includes(x.assignmentId)
    );
    const attend = db.attendance.filter(
          ...
          <tbody>
            ${subs
              .map((sub) => {
                ...
                <td>${sub.grade ?? "—"}</td>
                <td>${sub.feedback ?? "—"}</td>
                <td><button class="btn" data-grade="${
                  sub.id
                }">Grade</button></td>
              </tr>`;
              })
    ...
    `;
    byId("btnMessage").onclick = () => {
      state.ui.messages.studentId = studentId;
      navigate("messages");
    };
  ...
  const renderAttendance = () => {
    ...
    if (!courseId)
      return (byId("contentArea").innerHTML =
        '<div class="card"><div class="card-header"><h3>No courses assigned</h3></div></div>');
        ...
        <div class="toolbar">
          <div class="form-grid" style="grid-template-columns: 1fr 1fr 1fr;">
            <div class="field"><label>Course</label>
              <select id="attCourse">${state.myCourses
                .map(
                  (c) =>
                    `<option value="${c.id}" ${
    ...
    `;
    byId("attCourse").onchange = (e) => {
      state.ui.attendance.courseId = e.target.value;
      renderAttendance();
    };
    byId("attDate").onchange = (e) => {
      state.ui.attendance.date = e.target.value;
      renderAttendance();
    };
    byId("btnSaveAtt").onclick = () => {
      ...
      sels.forEach((sel) => {
        ...
        let rec = db.attendance.find(
          (a) =>
            a.courseId === courseId && a.date === date && a.studentId === sid
        );
        if (!rec) {
          rec = {
            id: uid("att"),
  ...
  const renderSubmissions = (assignmentId) => {
    ...
    byId("contentArea").innerHTML = `
      <div class="card">
        <div class="card-header"><h3>${a.title} (Due: ${fmtDate(
      a.dueDate
    )})</h3>
          <div class="table-actions">
            ...
            ${subs
              .map((sub) => {
                ...
                <td>
                  ...
                  <button class="btn" data-toggle-submit="${sub.id}">${
                  sub.submittedAt ? "Mark Unsubmitted" : "Mark Submitted"
                }</button>
                </td>
              </tr>`;
              })
    ...
    `;
    byId("btnBackToCourse").onclick = () =>
      navigate("course_detail", a.courseId);
    byId("contentArea").addEventListener(
      "click",
      (e) => {
        const g = e.target.closest("[data-grade]");
        const t = e.target.closest("[data-toggle-submit]");
        if (g) {
          const sub = db.submissions.find((x) => x.id === g.dataset.grade);
          openModal(
            "Grade Submission",
      ...
      [
        {
          ...
          type: "select",
          value: courseId ?? "",
          options: [{ value: "", label: "College-wide" }].concat(
  ...
  const renderAnnouncements = () => {
    ...
    const options = [{ value: "", label: "All" }].concat(
      state.myCourses.map((c) => ({ value: c.id, label: c.code }))
    );
    filter.innerHTML = `
      <div class="card">
        <div class="toolbar">
  ...
  const renderMessages = () => {
    ...
    const myStudents = [
      ...new Set(state.myCourses.flatMap((c) => c.studentIds)),
    ].map(getStudent);
    ...
    const loadThread = (sid) => {
      const thread = db.messages.find(
        (m) => m.studentId === sid && m.teacherId === LOGGED_IN_TEACHER_ID
      ) || { history: [] };
      const box = byId("chatMessages");
      box.innerHTML = thread.history
        .map(
          (m) =>
            `<div class="chat-bubble ${
              m.from === LOGGED_IN_TEACHER_ID ? "sent" : "received"
            }">${m.text}</div>`
        )
        .join("");
    ...
    byId("msgStudentList").addEventListener("click", (e) => {
      ...
      if (!item) return;
      $$("#msgStudentList .student-list-item").forEach((el) =>
    ...
    byId("chatSend").onclick = () => {
      ...
      saveDb();
      input.value = "";
      loadThread(state.ui.messages.studentId);
    };
    loadThread(activeId);
  };
  ...
  const renderReports = () => {
    ...
    const avgGrades = courses.map((c) => {
      const assnIds = db.assignments
        ...
        .map((a) => a.id);
      const grades = db.submissions
        .filter((s) => assnIds.includes(s.assignmentId) && s.grade != null)
        .map((s) => s.grade);
    ...
    chartInstances.chartGrades = new Chart(
      byId("chartGrades").getContext("2d"),
      {
        type: "bar",
        data: {
          labels,
          datasets: [{ label: "Average Grade", data: avgGrades }],
        },
        options: {
          responsive: true,
    ...
    chartInstances.chartAttendance = new Chart(
      byId("chartAttendance").getContext("2d"),
      {
        type: "line",
  ...
  const renderCalendar = () => {
    ...
    db.assignments.forEach((a) =>
      events.push({ date: a.dueDate, type: "assignment", title: a.title })
    );
    db.announcements.forEach((a) =>
      events.push({ date: a.date, type: "announcement", title: a.title })
    );
      ...
      <div class="card">
        <div class="toolbar">
          <div class="table-actions">
            ...
            <div style="font-weight:700">${d.toLocaleString(undefined, {
              month: "long",
              year: "numeric",
            })}</div>
        ...
        <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:.5rem;">
          ${["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]
            .map(
              (h) =>
                `<div style="text-align:center;color:var(--muted);font-weight:700;">${h}</div>`
            )
            .join("")}
          ${grid
            .map((cell) => {
                ...
                .map(
                  (ev) =>
                    `<div class="status-badge ${
                      ev.type === "assignment" ? "pending" : "graded"
                    }" style="display:inline-block;margin-top:.25rem;">${
                      ev.type === "assignment" ? "Due" : ""
                    } ${ev.title}</div>`
                )
                .join("")}
            </div>`;
  ...
  byId("themeToggle").addEventListener("click", () => {
    ...
    byId("themeToggle").innerHTML = `<span class="material-icons-outlined">${
